...
object AndroidInstall {
  ...
  private def installTask(emulator: Boolean) = (dbPath, packageApkPath) map {
    (dp, p) =>
      adbTask(dp.absolutePath, emulator, "install -r " + p.absolutePath)
  }

  private def uninstallTask(emulator: Boolean) = (dbPath, manifestPackage) map {
    (dp, m) =>
            ...
            val args =
              "-injars" :: classDirectory.absolutePath + sep +
                (if (!proguardInJars.isEmpty)
                  proguardInJars.map(_ + manifestr.mkString("(", ",!**/", ")")).mkString(sep)
                else "") ::
