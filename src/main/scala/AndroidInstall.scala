...
object AndroidInstall {
              ...
              val args =
                "-injars" :: classDirectory.absolutePath + JFile.pathSeparator +
                  ...
                  (if (!proguardInJars.isEmpty)
                    JFile.pathSeparator +
                    proguardInJars.map(_ + manifestr.mkString("(", ",!**/", ")")).mkString(JFile.pathSeparator)
                  else "") ::
  ...
  lazy val settings: Seq[Setting[_]] = inConfig(Android)(installerTasks ++
    aliasKeys.map(t ⇒ t <<= t dependsOn packageDebug) ++ Seq(
      ...
      packageRelease <<= packageTask(false)) ++ Seq(packageDebug, packageRelease).map {
        t ⇒
          t <<= t dependsOn (cleanApk, aaptPackage)
      }) ++ defaultAliases
}
